...
In addition to providing ways to [read](dev/server_api/v2/read_requests) and [write](dev/server_api/v2/write_requests) online library data, the Zotero Server API allows you to upload attachment files.
  ...
  "items": [
    {
      ...
      "linkMode": "imported_url",
      "title": "My Document",
      "accessDate": "2012-03-14 17:45:54",
...
Top-level attachments can be created by excluding the `parentItem` property. As in the Zotero client, only file attachments and PDFs—not HTML snapshots or web links—can be added as top-level items.

## 1b) Modify an existing attachment

### i. Retrieve the attachment information

    GET /users/<userID>/items/<itemKey>?content=json
