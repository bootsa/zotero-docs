...
TODO: attachment creation (see [File Uploads](dev/server_api/v2/file_upload))
...
The `PATCH` behavior is also available when [modifying multiple items](dev/server_api/v2/write_requests#updating_multiple_objects) via `POST`.
...
For syncing objects with predetermined keys, an object key can also be provided with new objects. See the [Syncing](dev/server_api/v2/syncing) documentation for more information.
