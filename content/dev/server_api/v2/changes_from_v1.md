...
As [announced on zotero-dev](https://groups.google.com/d/msg/zotero-dev/egIdfVmWEeo/dkOfupK7EXoJ), version 2 of the [Zotero Server API](dev/server_api/v2/start) brings many changes to enable [API-based syncing](dev/server_api/v2/syncing). While read-only clients may continue to work properly, clients that write to the API are likely to need updating.
...
-   New: [API versioning](dev/server_api/v2/read_requests#api_versioning)
-   New: [Conditional read support](dev/server_api/v2/read_requests#caching)
-   New: [Multi-object modification](dev/server_api/v2/write_requests#updating_multiple_objects) and [multi-object deletion](dev/server_api/v2/write_requests#deleting_multiple_items)
-   New: [Delta writes](dev/server_api/v2/write_requests#partial_item_updating_patch)
...
-   Object ETags have been replaced by [library/object versions](dev/server_api/v2/syncing#version_numbers). `If-Match` has been replaced by `If-Unmodified-Since-Version` and JSON version properties.
-   Object writes no longer return Atom feeds with the newly created objects. Single-object writes return `204 No Content`, while [multi-object writes](dev/server_api/v2/write_requests#creating_multiple_objects) return `200 OK` with a JSON document indicating the status of each write.
