...
The page documents read requests available in the [Zotero Web API](dev/web_api/v2/start), providing read-only access to online Zotero libraries.
...
To use this version of the API, include the HTTP header `Zotero-API-Version: 2` with all requests. **The current default version is [version 3](dev/web_api/v3).**
...
Accessing non-public data via the API requires use of an API key. Third-party developers should [use OAuth to establish credentials](dev/web_api/v2/oauth) or instruct their users to create dedicated keys for use with their services. End users can create API keys via [their Zotero account settings](/settings/keys/new).
...
The API also supports [write requests](dev/web_api/v2/write_requests) that allow the creation, modification, and deletion of data in online Zotero libraries.
...
[Library/object versioning](dev/web_api/v2/syncing#version_numbers) or [Zotero-Write-Token](dev/web_api/v2/write_requests#zotero_write_token) errors will result in `412 Precondition Failed` or `428 Precondition Required`.
