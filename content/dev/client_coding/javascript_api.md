...
Whereas Zotero's [Web API](dev/web_api) allows read and write access to online Zotero libraries, it is also possible to access the local Zotero client through the local JavaScript API. (It is also possible to [directly access the local SQLite database](dev/client_coding/direct_sqlite_database_access), but that approach is much more fragile.)
...
Windows in Zotero can import the core `Zotero` object via the include.js script. Zotero methods can then be called from anywhere within the window's scope simply by calling, for example, `var item = Zotero.Items.get(1);`.

To access Zotero functionality from your own extension, you will need access to the core `Zotero` object. If your extension operates within the main Zotero window, you already have access to the `Zotero` object and don’t need to take further steps. Otherwise, you can import the Zotero object into other windows by including the script chrome://zotero/content/include.js within an HTML or XUL file:
...
See the Zotero [Sample Plugin](dev/sample_plugin) for a demonstration.
...
The complete list of other fields available to search on is on the [search fields](dev/client_coding/javascript_api/search_fields) page.
...
specified in Zotero's preferences. We will in the following example create this
...
TODO: change the style. get stuff in other formats,
especially RTF
...
The JavaScript API provides a powerful way to script changes to your Zotero library. The common case of search-and-replace is accomplished easily using a basic script in the [JavaScript runner](#running_ad_hoc_javascript_in_zotero).

Before proceeding, back up your [Zotero data directory](zotero_data) and temporarily disable auto-sync in the Sync pane of the Zotero preferences.
...
<p id="zotero-5-update-warning" style="color: red; font-weight: bold">This example has not been updated for Zotero 5 and should not currently be used.</p>
